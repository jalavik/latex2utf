...
# Script that generates mappings between common LaTeX symbols and Unicode. 
# Taken from various sources, see credits.
...
# Credit: http://stackoverflow.com/questions/4578912/replace-all-accented-characters-by-their-latex-equivalent
# FX, http://stackoverflow.com/users/143495/fx
latexAccents = [
  [ u"à", "\\`a" ], # Grave accent
  [ u"è", "\\`e" ],
  [ u"ì", "\\`\\i" ],
...
def bylen(word1, word2):
    """ Sort comparison method for that compares by length """
    return len(word1[0]) - len(word2[0])

def main():
    # Credit: http://stackoverflow.com/questions/4578912/replace-all-accented-characters-by-their-latex-equivalent
    # Giuseppe Ottaviano, http://stackoverflow.com/users/554527/giuseppe-ottaviano
    translations = []
    for line in open('utf8ienc.dtx'):
        ...
        if m:
            ...
            translations.append((latex, unichr(int(codepoint, 16))))

    for un, la in latexAccents:
        translations.append((la, un))

    for un, la in unicode_map.items():
        if la[0] == '{' and la[-1] == '}':
            la = la[1:-1]
        if la[0] == '$' and la[-1] == '$':
            la = la[1:-1]
    ...
    # Sort by string length of LaTeX symbols, descending.
    # This is done to account for cases where sub-strings of keywords get matched.
    translations.sort(cmp=bylen, reverse=True)

    translations = tuple(translations)

    # Serialize object
    file_fd = open('latex2unicode.dat', 'w')
    pickle.dump(translations, file_fd)
    file_fd.close()

    # Pretty-print to Python source file
    pp = pprint.PrettyPrinter(indent=4)
    file_fd = open('latex2unicode.py', 'w')
    file_fd.write("# -*- coding: utf-8 -*-\ntuple = %s" % pp.pformat(translations))
    file_fd.close()

    # Print to KB file
    file_fd = open('latex-to-unicode.kb', 'w')
    for latex, uni in translations:
        file_fd.write(latex + "|--|" + uni.encode('utf-8') + '\n')
    file_fd.close()
